...
export function useMyTickets() {
  return useQuery({ queryKey: ['tickets', 'mine'], queryFn: () => ticketApi.listMine().then(r=> r.data) });
}
export function useTicket(id: string) {
  return useQuery({ queryKey: ['tickets', id], queryFn: () => ticketApi.get(id).then(r=> r.data), enabled: !!id });
}
